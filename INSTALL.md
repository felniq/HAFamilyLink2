...
2. Authenticate through its web UI (`http://<docker-host>:8098`), using the same two-port flow as the add-on (start the authentication on port 8098, finish the Google login via noVNC on port 6079).
3. Note the URL you will give the integration: `http://<docker-host>:8098`. If you set the container's `API_KEY` environment variable, enter its value in the integration's separate masked API-key field.
...
2. Add repository `https://github.com/felniq/HAFamilyLink2` with category **Integration**.
3. Search for **Google Family Link 2** and download the latest version.
4. Restart Home Assistant.

### Manual

1. Download `familylink2.zip` from the [latest release](https://github.com/felniq/HAFamilyLink2), or copy the `custom_components/familylink2` folder from the repository.
2. Extract or copy it into your Home Assistant `config/custom_components/` directory, so that `config/custom_components/familylink2/manifest.json` exists. Copy the folder in full: it contains subpackages (`auth/`, `client/`, `translations/`, `utils/`) and non-Python files (`services.yaml`, `strings.json`) that the integration needs; a partial copy will not load.
3. Restart Home Assistant.

## Configuration flow

Go to **Settings > Devices & Services > Add Integration** and search for **Google Family Link 2**.
...
|---|---|
| **Auto-detect (add-on or local file)** | Route A. The integration finds the auth service on its own, trying in order: the add-on resolved through the Supervisor, then `http://localhost:8098`, then the encrypted cookie file `/share/familylink2/cookies.enc`. |
...
Enter the auth server URL, for example `http://192.168.1.100:8098`. If the standalone container has `API_KEY` configured, enter that value in the separate masked API-key field; otherwise leave the field empty.
...
|---|---|---|---|
| Integration Name | `Google Family Link 2` | | Display name of the config entry. |
...
3. Open the auth web UI (add-on **Open Web UI**, or `http://<docker-host>:8098`), click **Start Authentication**, and finish the Google login through noVNC again.
...
### "Google Family Link 2" not found when adding the integration

- Check that `config/custom_components/familylink2/manifest.json` exists at exactly that path.
...
- Verify the auth service is running and healthy: `curl http://<host>:8098/api/health`.
...
- Route A: the key is read automatically from `/share/familylink2/api_key`; a 403 usually means the URL was entered manually without the key. Prefer auto-detect on Route A.
...
- Route A: check that `/share/familylink2/cookies.enc` exists.
...
- The first fetch runs during setup and data then refreshes every 60 seconds by default; check the logs (filter `familylink2`) for API errors.
- Top-app sensors stay unavailable until the child has used apps today.

## Uninstalling

1. **Settings > Devices & Services > Google Family Link 2**, three-dot menu, **Delete**.
2. Remove the integration files: via HACS (**Remove**) or by deleting `config/custom_components/familylink2/`, then restart Home Assistant.
...
4. Optionally delete the stored cookies: `/share/familylink2/` (Route A) or the container's `./data` directory (Route B).

## Getting help

Search the [existing issues](https://github.com/felniq/HAFamilyLink2) or open a new one. Include your Home Assistant version, the integration and auth container versions, and the relevant log lines (**Settings > System > Logs**, filter `familylink2`).
